...
import UpdateForm from './UpdateForm';
    ...
    class MyTable extends Component {
        ...
        updateField = async (id) => {
            id.preventDefault()
            const { task, start_date, supposed_end_date, stage, status, percent, end_date, _id } = this.state.kpis;
            // const { status } = this.state.kpis;
    
            // API call
            const data = { task, start_date, supposed_end_date, stage, status, percent, end_date, _id };
            ...
            //             // console.log('kpis id', id.target)
            // }

            // console.log("handleChange name -", name)
            // console.log("handleChange value -", value)
            // try {
            //    let result = await axios({ method : 'PUT', url: `http://localhost:5000/kpis/${id}`, data });
